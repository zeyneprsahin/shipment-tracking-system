...
**Validation ve Hata Yönetimi: Kullanıcıdan gelen alıcı adı, adres, telefon ve paket bilgisi gibi zorunlu alanların boş olup olmadığı API/Application tarafında kontrol edilecek. Gönderinin hangi durumdan hangi duruma geçebileceği ise bir iş kuralı olduğu için Domain katmanında kontrol edilecek.

Geçersiz bir durum geçişi yapılmak istenirse Domain katmanı DomainRuleException fırlatacak. API tarafında merkezi bir exception middleware bu hatayı yakalayacak ve istemciye 409 Conflict döndürecek. Gönderi bulunamazsa 404 Not Found, eksik veya hatalı veri varsa 400 Bad Request, beklenmeyen bir hata olursa 500 Internal Server Error dönecek. Böylece controller içinde her işlem için ayrı try-catch yazmayacağım ve hata cevapları tek bir yerde yönetilecek.

Örneğin Teslim Edildi durumundaki bir gönderi Hazırlanıyor durumuna çekilmeye çalışılırsa Domain bunu kabul etmeyecek; istemci 409 Conflict ve geçişin neden geçersiz olduğunu belirten bir hata mesajı görecek.
